...
그렇다면 테스트용 UserService 확장 클래스는 어떻게 만들까? UserService를 복붙한 뒤 일부를 수정하는 것은 중복도 발생하고 번거로워 지므로, 간단히 UserService를 상속해서 테스트에 필요한 기능을 추가하도록 일부 메소드를 오버라이딩하는 방법이 낫다.
...
테스트용 서브 클래스에서 오버라이딩할 메소드는 upgradeLevel() 메소드다. 따라서 이 메소드의 접근 권한을 private 에서 protected로 변경하자.
